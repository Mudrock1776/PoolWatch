...
char *WIFI_SSID = "Joewifi";
char *WIFI_PASSWORD = "kns4rumh3u686wn";
...
// How many extra times to retry particulate capture
static const int PARTICULATE_MAX_RETRIES = 3;
...
void loop() {
  ...
  pumpStatus = getPumpHealth();  //tie to both 12V regulator being ok and if pump ever ran
  ...
  //   CaptureParsed res = slave.captureOnce(/*wakeLowMs=*/1000);
  ...
  if ((statusOutput & 8) == 8) {
    ...
    // Start from a clean state so we don’t reuse any stale payload
    slave.clearPayload();

    CaptureParsed res;
    int attempt;
    for (attempt = 0; attempt <= PARTICULATE_MAX_RETRIES; ++attempt) {

      res = slave.captureOnce(/*wakeLowMs=*/1000);

      // Consider payload invalid if:
      //  - not ok
      //  - any string empty
      //  - OR all three percentages are 0.00%
      bool stringsPresent =
        (res.s1.length() > 0) && (res.s2.length() > 0) && (res.s3.length() > 0);

      bool allZeroPercent =
        (res.s1.indexOf(" 0.00%") != -1) && (res.s2.indexOf(" 0.00%") != -1) && (res.s3.indexOf(" 0.00%") != -1);

      bool valid =
        res.ok && stringsPresent && !allZeroPercent;

      if (valid) {
        if (DEBUG) {
          Serial.print("[ESP] Particulate capture attempt ");
          Serial.print(attempt + 1);
          Serial.println(" succeeded");
        }
        break;  // accept this capture
      }
      ...
      if (DEBUG) {
        ...
        Serial.print(attempt + 1);
        Serial.print(" failed (");
        Serial.print(res.error);
        if (allZeroPercent) {
          Serial.print(", allZeroPercent");
        }
        Serial.println("), retrying...");
      }
      // Loop will try again up to PARTICULATE_MAX_RETRIES
    }

    // Final validity check uses the same rules as above
    bool stringsPresent =
      (res.s1.length() > 0) && (res.s2.length() > 0) && (res.s3.length() > 0);
    ...
    if (!finalValid) {
      // Still bad after all retries
      PartAmount = 0;
      PartSize = "Particulate capture failed";
    } else {
      // Good result
      PartAmount = res.count;
      PartSize = res.s1 + ", " + res.s2 + ", " + res.s3;
