    ...
    <div className={`w-full max-w-sm mx-auto rounded-lg border-2 border-gray-400 dark:border-gray-500 shadow-lg bg-white dark:bg-gray-800 ${className}`}>
      <div className="p-5 md:p-8 rounded-t flex flex-col" style={{ height }}>
        <div className="px-1 flex items-center justify-between h-6">
          <span className="whitespace-nowrap text-base font-bold text-gray-800 dark:text-gray-100">{monthLabel}</span>
          <div className="flex items-center">
            <button
              ...
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" fill="none" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 6 9 12 15 18" /></svg>
            </button>
            <button
              type="button"
        ...
        <div className="mt-2 grid grid-cols-7 gap-1 text-xs text-gray-500 h-5 select-none">{weekdayLabels.map((weekday) => (<div key={weekday} className="text-center">{weekday}</div>))}</div>
        <div className="mt-1 grid grid-cols-7 grid-rows-6 gap-1 flex-1 overflow-hidden">
          {calendarDays.map((date, index) => {
            ...
            if (selected) {
              ...
              cellClass += 'ring-1 ring-blue-600 ';
            } else {
            ...
            return (
              <button
                type="button"
                key={isoDate + index}onClick={() => {setSelectedIsoDate(isoDate);if (onSelectDate) onSelectDate(isoDate);}}className={cellClass}title={isoDate}>{date.getDate()}
              </button>
            );
          })}
        </div>
