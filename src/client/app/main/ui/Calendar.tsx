...
function startOfWeek(date: Date, mondayFirst: boolean) {
  ...
  let dayOfWeekIndex = dateCopy.getDay(); // Sunday=0 
  if (mondayFirst) dayOfWeekIndex = (dayOfWeekIndex + 6) % 7; // Monday=0 
...
export default function Calnder({
    ...
    return (
      <div className={`w-full max-w-sm mx-auto rounded-lg border-2 border-gray-400 dark:border-gray-500 shadow-lg bg-white dark:bg-gray-800 ${className}`}>
        <div className="p-5 md:p-8">
          <div className="h-5 w-32 bg-gray-2 00 dark:bg-gray-700 rounded mb-3 animate-pulse" />
  ...
  const weekdayLabels = startOnMonday ? ['Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa', 'Su'] : ['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'];
        ...
        <div className="mt-1 grid grid-cols-7 grid-rows-6 gap-1 flex-1 overflow-hidden">
          {calendarDays.map((date, index) => {
            ...
            const classNames =
              ...
              (isInCurrentMonth(date) ? 'text-gray-900 dark:text-gray-100' : 'text-gray-400') +
              (isSelected
