...
unsigned long phosphateWait = 90000; // 1 min 30 sec
...
bool chlorineSequenceRan = false;  // track if chlorine sequence ran this cycle
...
void runPhosphateSequence() {
  ...
  //Wait 1 min 30 b/f adding next reagent
  if (phosphateWait > 0) {
    if (DEBUG) {
  ...
  if (DEBUG) {
  Serial.println("Extra 30 sec delay before second phosphate reagent...");
  }
  delay(30000);   // 30 sec
...
void loop() {
  ...
  if(statusOutput[0] != 0){
    ...
    chlorineSequenceRan = false;
    if (statusOutput[4]){

      //check if chlorine sequence ran (checks if cuvette filled inside already)
      if (!chlorineSequenceRan) {
        runChlorineSequence();
        chlorineSequenceRan = true;
      }
    ...
    if (statusOutput[1]){
      //check if chlorine sequence ran before
      if (!chlorineSequenceRan) {
        runChlorineSequence();
        chlorineSequenceRan = true;
      }
